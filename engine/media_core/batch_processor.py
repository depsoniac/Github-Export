...
from media_core.downloader import download_media, apply_site_specific_rules, apply_yt_patch
from media_core.exceptions import UserCancelledError
from media_core.video_upscaler import VideoUpscaler
from media_core.paths import UPSCALING_DIR
...
def get_smart_thumbnail_extension(image_data):
    """
    Detecta el formato Ã³ptimo para guardar la miniatura:
    - PNG si tiene transparencia
    - JPG en otros casos (mÃ¡s compacto)
    """
    try:
        from PIL import Image
        import io
...
class Job:
    """
    Contiene la informaciÃ³n y el estado de un Ãºnico trabajo en la cola.
    """
...
class QueueManager:
    ...
    """
    def __init__(self, host_adapter, event_callback):
    ...
    def start_worker_thread(self):
        """Inicia el hilo de trabajo si no estÃ¡ ya corriendo."""
    ...
    def _worker_thread(self):
        ...
        while not self.stop_event.is_set():
            ...
            if job_to_run:
                try:
                    # --- INICIO DE MODIFICACIÃ“N: LÃ³gica de enrutamiento ---
                    ...
                    self.event_callback(job_to_run.job_id, "PENDING", f"Pausado: {e}")
                
                except Exception as e:
                    print(f"ERROR: FallÃ³ el trabajo {job_to_run.job_id}: {e}")
                    job_to_run.status = "FAILED"
                    self.event_callback(job_to_run.job_id, "FAILED", f"Error: {str(e)[:100]}")
                
                # --- INICIO DE MODIFICACIÃ“N (Progreso Global) ---
                ...
                if job_to_run.status not in ("PENDING", "RUNNING"):
                    with self.jobs_lock:
                        # Contar todos los trabajos que ya no estÃ¡n en la cola de espera
                    ...
                    if total_jobs > 0:
                        ...
                        # Mensaje de progreso
                        current_title = job_to_run.config.get('title', 'Ãtem')
                        ...
                        if job_to_run.status == "FAILED":
                            progress_message = f"({self.jobs_completed}/{total_jobs}) FallÃ³: {current_title}"
                        elif job_to_run.status == "SKIPPED":
                            progress_message = f"({self.jobs_completed}/{total_jobs}) Omitido: {current_title}"
                        
                        self.event_callback("GLOBAL_PROGRESS", "UPDATE", progress_message, progress_percent)
                # --- FIN DE MODIFICACIÃ“N ---
            
            else:
                # No hay trabajos pendientes
                batch_context = self.host_adapter.batch_context
                if batch_context:
                    if not batch_context.auto_download_checkbox.get():
                        # Auto-descarga estÃ¡ OFF. Pausar la cola automÃ¡ticamente.
                        if not self.pause_event.is_set():
                            print("INFO: Cola completada. Auto-descargar deshabilitado, pausando...")
                            self.pause_event.set()
                            self.user_paused = False # <-- NO fue el usuario
                            self.event_callback("QUEUE_STATUS", "PAUSED", "")
                    else:
                        # Auto-descarga estÃ¡ ON. La cola simplemente espera.
                        # Si llegamos aquÃ­, la cola estÃ¡ inactiva (sin trabajos)
    ...
    def add_job(self, job: Job):
        """AÃ±ade un nuevo trabajo a la cola y notifica a la UI."""
        with self.jobs_lock:
            self.jobs.append(job)
            print(f"INFO: Nuevo trabajo aÃ±adido a la cola: {job.config.get('title', job.job_id)}")
        
        self.event_callback(job.job_id, "PENDING", job.config.get('title', 'Trabajo pendiente...'))
    ...
    def start_queue(self):
        ...
        self.event_callback("QUEUE_STATUS", "RUNNING", "")

        # OK CORRECCIÃ“N: Forzar actualizaciÃ³n inmediata de la barra
        # Esto elimina el "100%" residual del anÃ¡lisis y pone la barra en 0% (o en el estado actual)
        with self.jobs_lock:
            total_jobs = len(self.jobs)
        ...
        if total_jobs > 0:
            ...
            self.event_callback("GLOBAL_PROGRESS", "UPDATE", msg, progress_percent)
        else:
            self.event_callback("GLOBAL_PROGRESS", "RESET", "Esperando...", 0.0)
    ...
    def remove_job(self, job_id: str):
        ...
        with self.jobs_lock:
            ...
            if job_to_remove:
                ...
                print(f"ADVERTENCIA: Se intentÃ³ eliminar el job {job_id} pero no se encontrÃ³.")
    ...
    def reset_progress(self):
        ...
        self.event_callback("GLOBAL_PROGRESS", "RESET", "Esperando para iniciar la cola...", 0.0)

    def _execute_playlist_job(self, job: Job):
        """
        Procesa una playlist completa. Maneja lÃ³gica de 'Solo Miniaturas' 
        ...
        batch_context = self.host_adapter.batch_context
        thumbnail_mode = batch_context.thumbnail_mode_var.get() # 'normal', 'with_thumbnail', 'only_thumbnail'
        conflict_policy = batch_context.conflict_policy_menu.get() if hasattr(batch_context, 'conflict_policy_menu') else "Renombrar"
        
        # Directorio base y subcarpeta
        base_output_dir = batch_context.output_path_entry.get()
        ...
        playlist_title = self.host_adapter.single_context.sanitize_filename(raw_title)
        ...
        self.event_callback(job.job_id, "RUNNING", f"Iniciando playlist ({total_videos} items)...")

        # --- CASO ESPECIAL: SOLO MINIATURAS ---
        if thumbnail_mode == "only_thumbnail":
            # Crear carpeta especÃ­fica de thumbnails dentro de la playlist (opcional, o usar la raÃ­z de la playlist)
            # El usuario pidiÃ³ "descargue las miniaturas de la playlist", lo pondremos en la carpeta de la playlist.
            ...
            for i, index in enumerate(selected_indices):
                ...
                self.event_callback(job.job_id, "RUNNING", msg, percent)
                ...
                try:
                    ...
                    # 2. Verificar si Auto-envÃ­o estÃ¡ activo
                    if thumb_path and self.host_adapter.batch_context.auto_send_to_it_checkbox.get() == 1:
                        # 3. Enviar a Herramientas de Imagen (Usando after para seguridad de hilos)
                        self.host_adapter.dispatch(0, self.host_adapter.image_context._process_imported_files, [thumb_path])
            ...
            self.event_callback(job.job_id, "COMPLETED", "Miniaturas descargadas OK", 100.0)
            return

        # --- CASO NORMAL (VIDEO/AUDIO) ---
        for i, index in enumerate(selected_indices):
            if self.pause_event.is_set():
                self.event_callback(job.job_id, "PENDING", f"Pausado en video {i+1}/{total_videos}")
                return 
            if self.stop_event.is_set(): return
            ...
            def playlist_progress_callback(vid_percent, vid_message):
                ...
                self.event_callback(job.job_id, "RUNNING", status_msg, global_percent)
            ...
            child_options = {
                ...
                'mode': mode,
                'cookie_mode': self.host_adapter.cookies_mode_saved,
            }
            
            self._apply_playlist_quality(child_options, mode, quality_setting)
            
            # Inicializar variables para importaciÃ³n
            thumb_path = None
            ...
            try:
                ...
                # OK ROBUSTEZ: Corregir ruta si cambiÃ³ la extensiÃ³n (ej: Solo Audio)
                ...
                # OK LÃ“GICA DE HERENCIA DE RECODIFICACIÃ“N
                if job.config.get('recode_enabled', False) and downloaded_path and os.path.exists(downloaded_path):
                    ...
                    if preset_params:
                        ...
                        # OK CORRECCIÃ“N: Inyectar el modo de la playlist en las opciones
                        recode_options = preset_params.copy()
                        recode_options['mode'] = mode 

                        # --- INTERCEPCIÃ“N DE EXTRAS ---
                        ...
                        if mode != "Solo Audio" and (is_extraction or is_upscaling):
                            if is_extraction:
                                self.event_callback(job.job_id, "RUNNING", f"[{i+1}/{total_videos}] Fotogramas: {video_title}...")
                                ...
                                output_folder = self.host_adapter.ffmpeg_processor.execute_video_to_images(
                                    extraction_options,
                                    lambda p, m: self.event_callback(job.job_id, "RUNNING", f"[{i+1}/{total_videos}] Extraer {p:.1f}%"),
                                    self.pause_event
                                )
                                ...
                                self.event_callback(job.job_id, "RUNNING", f"[{i+1}/{total_videos}] Reescalando: {video_title}...")
                                ...
                                if out_path:
                                    ffmpeg_dir = os.path.dirname(self.host_adapter.ffmpeg_processor.ffmpeg_path)
                                    upscaler = VideoUpscaler(
                                        ffmpeg_dir=ffmpeg_dir,
                                        upscaling_dir=UPSCALING_DIR,
                                        cancellation_event=self.pause_event,
                                        progress_callback=lambda p, m: self.event_callback(job.job_id, "RUNNING", f"[{i+1}/{total_videos}] {m} ({p:.1f}%)" if isinstance(p, float) and p >= 0 else f"[{i+1}/{total_videos}] {m}")
                                    )
                        ...
                        # Si no se procesÃ³ por extra, ejecutar recodificaciÃ³n normal
                        if not processed_by_extra:
                            self.event_callback(job.job_id, "RUNNING", f"[{i+1}/{total_videos}] Recodificando: {video_title}...")
                ...
                if thumbnail_mode == "with_thumbnail":
                    ...
                    # OK LÃ³gica de Auto-EnvÃ­o para Ã­tems de Playlist
                    if thumb_path and self.host_adapter.batch_context.auto_send_to_it_checkbox.get() == 1:
                        self.host_adapter.dispatch(0, self.host_adapter.image_context._process_imported_files, [thumb_path])
                    
                # 4. OK LÃ“GICA DE IMPORTACIÃ“N AUTOMÃTICA A ADOBE (NUEVO)
                if self.host_adapter.batch_context.auto_import_checkbox.get() and final_path_for_import:
                    active_target = self.host_adapter.ACTIVE_TARGET_SID_accessor()
                    ...
                    if active_target:
                        ...
                        # Por simplicidad, usaremos el tÃ­tulo de la playlist para agrupar los Ã­tems
                        
                        file_package = {
                            "video": final_path_for_import.replace('\\', '/'),
                            "thumbnail": thumb_path.replace('\\', '/') if thumb_path else None,
                            "subtitle": None, # (PodrÃ­as aÃ±adir lÃ³gica de subtÃ­tulos aquÃ­ en el futuro)
                            "targetBin": target_bin_name
                        }
                        
                        print(f"INFO: [Playlist Item] Enviando a Adobe: {os.path.basename(final_path_for_import)}")
                        self.host_adapter.socketio.emit('new_file', {'filePackage': file_package}, to=active_target)
        ...
        job.status = "COMPLETED"
        job.final_filepath = playlist_dir
        self.event_callback(job.job_id, "COMPLETED", "Playlist completada OK", 100.0)

    def _apply_playlist_quality(self, options, mode, quality_setting):
        """Traduce la selecciÃ³n del menÃº a selectores de formato de yt-dlp."""
        ...
        if mode == "Video+Audio":
            ...
            if "Mejor Compatible" in quality_setting:
                ...
                # Si no existe, descargarÃ¡ el mejor y luego convertiremos (post-proceso)
                selector = "bestaudio[ext=m4a]/bestaudio[ext=mp3]/bestaudio"
                options['recode_audio_enabled'] = True # Forzar recodificaciÃ³n si es necesario
    ...
    def _download_single_video_in_playlist(self, options, progress_callback, job_id):
        """
        VersiÃ³n mini de _execute_download_job para uso interno en playlists.
        """
        output_dir = options['output_path']
        title = self.host_adapter.single_context.sanitize_filename(options['title'])
        ...
        ydl_opts = {
            ...
            'quiet': True,
            'no_warnings': True,
            'ffmpeg_location': self.host_adapter.ffmpeg_processor.ffmpeg_path
        }
        ...
        if cookie_mode == "Archivo Manual..." and self.host_adapter.cookies_path:
            ydl_opts['cookiefile'] = self.host_adapter.cookies_path
            using_cookies = True
        elif cookie_mode != "No usar":
            browser = self.host_adapter.selected_browser_saved
            profile = self.host_adapter.browser_profile_saved
            if profile:
        ...
        try:
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                # OK CAMBIO: Capturar info para obtener el nombre real del archivo
                info = ydl.extract_info(options['url'], download=True)
                
                # Obtener la ruta final del archivo descargado
                filename = ydl.prepare_filename(info)
                return filename # <--- AÃ‘ADIR ESTE RETURN
    ...
    def _execute_download_job(self, job: Job):
        """
        Ejecuta un Ãºnico trabajo de DESCARGA (desde URL).
        """
        self.event_callback(job.job_id, "RUNNING", "Iniciando...")
        batch_context = self.host_adapter.batch_context
        
        # Verificar modo de descarga global
        thumbnail_mode = batch_context.thumbnail_mode_var.get()
        ...
        single_context = self.host_adapter.single_context
        
        output_dir = batch_context.output_path_entry.get()
        if not output_dir:
        ...
        conflict_policy = batch_context.conflict_policy_menu.get()
        speed_limit = batch_context.speed_limit_entry.get()
        
        url = job.config.get('url')
        title = single_context.sanitize_filename(job.config.get('title', 'video_lote'))
        mode = job.config.get('mode', 'Video+Audio')
        v_label = job.config.get('video_format_label', '-')
        a_label = job.config.get('audio_format_label', '-')

        # Si no tenemos anÃ¡lisis completo, hacerlo ahora
        if not job.analysis_data or 'formats' not in job.analysis_data:
            self.event_callback(job.job_id, "RUNNING", "Analizando formatos...")
            try:
                ydl_opts = {
                    'no_warnings': True,
                    'noplaylist': True,
                ...
                if cookie_mode == "Archivo Manual..." and self.host_adapter.cookies_path:
                    ...
                    browser_arg = self.host_adapter.selected_browser_saved
                    profile = self.host_adapter.browser_profile_saved
                    if profile:
        ...
        if not job.analysis_data or 'formats' not in job.analysis_data:
            raise Exception("No se pudo extraer informaciÃ³n de formatos del video")
        
        extractor_key = job.analysis_data.get('extractor_key', '').lower()

        # TWITCH: Deshabilitar subtÃ­tulos (no funciona bien)
        if 'twitch' in extractor_key:
            print("DEBUG: Twitch detectado, deshabilitando subtÃ­tulos")
            job.analysis_data['subtitles'] = {}
            job.analysis_data['automatic_captions'] = {}

        # TWITTER/X: Usar mejor opciÃ³n disponible
        ...
        if mode == "Solo Audio" and not job_audio_formats:
            ...
            self.event_callback(job.job_id, "NO_AUDIO", "Este Ã­tem no tiene audio.")
            return # Salir de la ejecuciÃ³n del job

        # --- INICIO DE LA CORRECCIÃ“N 1 ---
        # LÃ³gica de fallback mejorada para encontrar el formato "âœ¨"
        
        if v_label == "-" or v_label not in job_video_formats:
            v_opts = list(job_video_formats.keys())
            if v_opts:
                default_video_selection = v_opts[0] # Fallback por si no hay "âœ¨"
        ...
        # ðŸ†• Prioridad 1: IDs guardados explÃ­citamente
        v_id = job.config.get('resolved_video_format_id')
        a_id = job.config.get('resolved_audio_format_id')

        # ðŸ†• Fallback: Extraer de los formatos si no hay IDs guardados
        if not v_id:
        ...
        # ðŸ†• Si es combinado multiidioma, usar el ID del idioma seleccionado
        ...
        if is_combined and v_format_dict:
            ...
            batch_context = self.host_adapter.batch_context
            if quality_key and hasattr(batch_context, 'combined_audio_map') and batch_context.combined_audio_map:
                if a_label in batch_context.combined_audio_map:
                    v_id = batch_context.combined_audio_map[a_label]
        ...
        # ðŸ†• FALLBACK INTELIGENTE: Si no hay selector precisÃ³
        if not precise_selector:
            self.event_callback(job.job_id, "RUNNING", "Selector no especificado, usando fallback...")
        ...
        # OK CORRECCIÃ“N DINÃMICA: Extraer audio respetando el formato original
        if mode == "Solo Audio":
            # 1. Determinar el formato de destino basado en el cÃ³dec original
            target_ext = 'mp3' # Fallback por compatibilidad
            
            if a_format_dict:
                acodec = a_format_dict.get('acodec', '').lower()
                
                # Mapeo de cÃ³decs a extensiones para extracciÃ³n sin pÃ©rdidas (Copy)
        ...
        if playlist_index is not None:
            # Le dice a yt-dlp que SOLO descargue este Ã­tem
            ydl_opts['playlist_items'] = str(playlist_index)
            print(f"DEBUG: Configurando --playlist-items para {playlist_index}")
        else:
            # Esto no es de una playlist, asÃ­ que forzamos 'noplaylist'
            ydl_opts['noplaylist'] = True
            print(f"DEBUG: Configurando --noplaylist (video Ãºnico)")
        ...
        def download_hook(d):
            ...
            if d['status'] == 'downloading':
                ...
                if total > 0:
                    ...
                    self.event_callback(job.job_id, "RUNNING", f"Descargando... {percentage:.1f}% ({speed_str})")
            
            elif d['status'] == 'finished':
                self.event_callback(job.job_id, "RUNNING", "Descarga finalizada. Procesando...")
        ...
        try:
            ...
            # Limpiar backup si todo saliÃ³ bien
            if backup_path and os.path.exists(backup_path):
                os.remove(backup_path)
            
            thumbnail_path = None # Inicializar
            
            # Descargar miniatura si estÃ¡ habilitado
            if should_download_thumbnail:
                thumbnail_path = self._download_thumbnail_alongside_video(job, final_filepath)

            # OK CORRECCIÃ“N: Actualizar ruta final si cambiÃ³ la extensiÃ³n (Modo Solo Audio)
            # Si yt-dlp convirtiÃ³ el video a audio y borrÃ³ el original, 'final_filepath' apunta a la nada.
            if mode == "Solo Audio" and not os.path.exists(final_filepath):
                ...
                # 1. Intentar con la extensiÃ³n objetivo calculada antes (target_ext)
                ...
                # 2. Si sigue sin existir, buscar cualquier extensiÃ³n de audio comÃºn
                if not os.path.exists(final_filepath):
                    for audio_ext in ['.m4a', '.mp3', '.opus', '.wav', '.flac', '.ogg']:
                        candidate = f"{base_path_no_ext}{audio_ext}"
                        if os.path.exists(candidate):
                            final_filepath = candidate
                            print(f"DEBUG: Ruta corregida por bÃºsqueda (Audio): {final_filepath}")
                            break

            # --- INICIO DE LA LÃ“GICA DE RECODIFICACIÃ“N ---
            
            if job.config.get('recode_enabled', False):
                self.event_callback(job.job_id, "RUNNING", "RecodificaciÃ³n en cola...")
                
                preset_name = job.config.get('recode_preset_name')
                if not preset_name or preset_name.startswith('-'):
                    raise Exception("Preset de recodificaciÃ³n no vÃ¡lido seleccionado.")
                
                preset_params = self._find_preset_params(preset_name)
                if not preset_params:
                    raise Exception(f"No se encontraron parÃ¡metros para el preset '{preset_name}'.")
                ...
                if mode != "Solo Audio" and (is_extraction or is_upscaling):
                    if is_extraction:
                        self.event_callback(job.job_id, "RUNNING", "Extrayendo fotogramas...")
                        folder_name = f"{base_name}_frames"
                        final_output_directory = os.path.join(output_dir, folder_name)
                        ...
                        # Usa execute_video_to_images que ya estÃ¡ disponible en ffmpeg_processor
                        output_folder = self.host_adapter.ffmpeg_processor.execute_video_to_images(
                            extraction_options,
                            lambda p, m: self.event_callback(job.job_id, "RUNNING", f"Extrayendo... {p:.1f}%"),
                            self.pause_event
                        )
                        ...
                        self.event_callback(job.job_id, "COMPLETED", f"Completado (Fotogramas): {folder_name}")
                        return
                    
                    elif is_upscaling:
                        self.event_callback(job.job_id, "RUNNING", "Reescalando video (IA)...")
                        scale_str = str(preset_params.get("upscale_scale", "2")).replace("x", "")
                        out_stem = f"{base_name}_upscaled_x{scale_str}"
                        desired_out_path = os.path.join(output_dir, out_stem + ".mp4")
                        ...
                        if out_path is None:
                            job.status = "SKIPPED"
                            self.event_callback(job.job_id, "SKIPPED", "Omitido: El archivo reescalado ya existe")
                            return
                        ...
                        upscaler = VideoUpscaler(
                            ...
                            upscaling_dir=UPSCALING_DIR,
                            cancellation_event=self.pause_event,
                            progress_callback=lambda p, m: self.event_callback(job.job_id, "RUNNING", f"({p:.1f}%) {m}" if isinstance(p, float) and p >= 0 else f"{m}")
                        )
                        ...
                        self.event_callback(job.job_id, "COMPLETED", f"Completado (Upscaled): {os.path.basename(final_path)}")
                        return
                # --- FIN INTERCEPCIÃ“N DE EXTRAS ---

                recoded_base_name = f"{base_name}_recoded"

                # Ejecutar la recodificaciÃ³n
                ...
                if not job.config.get('recode_keep_original', True):
                    try:
                        ...
                        # Si se borrÃ³ el original, renombrar la miniatura si existe
            ...
            # --- FIN DE LA LÃ“GICA DE RECODIFICACIÃ“N ---

            job.status = "COMPLETED"
            job.final_filepath = final_filepath # OK Ahora apunta al archivo real (.m4a/.mp3)
            self.event_callback(job.job_id, "COMPLETED", f"Completado: {os.path.basename(final_filepath)}")

            # --- LÃ“GICA DE IMPORTACIÃ“N AUTOMÃTICA ---
            
            # 1. Verificar si el usuario quiere importar
            batch_context = self.host_adapter.batch_context
            # (Usamos tu nuevo texto "Import Pr/Ae" para encontrar el checkbox)
            if batch_context and batch_context.auto_import_checkbox.get():
                
                # 2. Verificar si hay una extensiÃ³n de Adobe conectada
                active_target = self.host_adapter.ACTIVE_TARGET_SID_accessor()
                if active_target:
                    ...
                    if hasattr(self, 'subfolder_path') and self.subfolder_path:
                        # self.subfolder_path es la RUTA COMPLETA (ej: C:/.../ClipDock List 01)
                    ...
                    file_package = {
                        ...
                        "subtitle": None,
                        "targetBin": target_bin_name # <-- AÃ‘ADIDO
                    }
            ...
            # Si fallÃ³, restaurar el backup si existÃ­a
            if backup_path and os.path.exists(backup_path):
    ...
    def _execute_recode_job(self, job: Job):
        """
        Ejecuta un Ãºnico trabajo de RECODIFICACIÃ“N LOCAL (desde archivo).
        """
        ...
        final_filepath = None
        try:
            self.event_callback(job.job_id, "RUNNING", "Iniciando recodificaciÃ³n local...")
            
            # 1. Verificar si la recodificaciÃ³n estÃ¡ activada
            if not job.config.get('recode_enabled', False):
                job.status = "SKIPPED"
                self.event_callback(job.job_id, "SKIPPED", "Omitido: La recodificaciÃ³n no estÃ¡ activada.")
                return
            ...
            if not input_file or not os.path.exists(input_file):
                raise Exception(f"No se encontrÃ³ el archivo local: {input_file}")
            ...
            if is_extraction:
                ...
                if os.path.exists(final_output_directory):
                    if conflict_policy == "Omitir":
                        job.status = "SKIPPED"
                        self.event_callback(job.job_id, "SKIPPED", "Omitido: La carpeta de fotogramas ya existe.")
                        return
                ...
                job.status = "COMPLETED"
                job.final_filepath = output_folder
                self.event_callback(job.job_id, "COMPLETED", f"Completado (Fotogramas): {os.path.basename(final_output_directory)}")
                return
            ...
            # 6. Ejecutar la recodificaciÃ³n (usando la misma funciÃ³n maestra)
            recoded_filepath = self._execute_recode_master(
                job=job,
                input_file=input_file,
                output_dir=output_dir,
            ...
            self.event_callback(job.job_id, "COMPLETED", f"Recodificado: {os.path.basename(final_filepath)}")

            # 9. LÃ³gica de ImportaciÃ³n AutomÃ¡tica (adaptada para locales)
            thumbnail_path = None
            if batch_context and batch_context.auto_import_checkbox.get():
                active_target = self.host_adapter.ACTIVE_TARGET_SID_accessor()
                if active_target:
                    
                    # Generar una miniatura sobre la marcha para la importaciÃ³n
                    try:
                        print(f"DEBUG: Generando miniatura para importaciÃ³n automÃ¡tica...")
                        duration = self._get_job_media_duration(job, final_filepath)
                        temp_thumb_path = self.host_adapter.ffmpeg_processor.get_frame_from_video(final_filepath, duration)
                        ...
                        if temp_thumb_path and os.path.exists(temp_thumb_path):
                            ...
                            print(f"DEBUG: Miniatura generada para importaciÃ³n: {thumbnail_path}")
                    except Exception as e:
            ...
            # Si falla, la excepciÃ³n serÃ¡ capturada por _worker_thread
            print(f"ERROR: FallÃ³ el trabajo de recodificaciÃ³n local {job.job_id}: {e}")
            raise e # Re-lanzar para que el worker lo marque como FAILED
        
    def _download_thumbnail_only(self, job: Job):
        """
        Descarga Ãºnicamente la miniatura del video.
        """
        try:
            batch_context = self.host_adapter.batch_context
            single_context = self.host_adapter.single_context
            ...
            if not thumbnail_url:
                raise Exception("No se encontrÃ³ miniatura para este video")
            
            self.event_callback(job.job_id, "RUNNING", "Descargando miniatura...")
            ...
            smart_ext = get_smart_thumbnail_extension(image_data) # <-- USAR NUEVA FUNCIÃ“N
            
            # Nombre del archivo
            title = single_context.sanitize_filename(job.config.get('title', 'thumbnail'))
            ...
            if final_path is None:
                ...
                self.event_callback(job.job_id, "SKIPPED", "Omitido: Miniatura ya existe")
            ...
            try:
                ...
                print(f"ERROR: FallÃ³ el procesamiento de la imagen (PIL): {pil_e}")
                # Fallback: guardar el archivo original (podrÃ­a fallar la importaciÃ³n)
                with open(final_path, 'wb') as f:
                    f.write(image_data)
            
            # Limpiar backup si existÃ­a
            if backup_path and os.path.exists(backup_path):
                os.remove(backup_path)
            
            job.status = "COMPLETED"
            job.final_filepath = final_path
            job.thumbnail_path = final_path # OK NUEVO: Guardar ruta explÃ­citamente
            self.event_callback(job.job_id, "COMPLETED", f"Miniatura guardada: {os.path.basename(final_path)}")
            ...
            if batch_context and batch_context.auto_import_checkbox.get():
                ...
                if active_target:
                    ...
                    # 3. Determinar el nombre del bin (LÃ“GICA CORREGIDA)
                    base_bin_name = None
                    ...
                    # Si habÃ­a una carpeta de lote, crear un nombre combinado
                    if base_bin_name:
    ...
    def _download_thumbnail_alongside_video(self, job: Job, video_filepath: str):
        """
        Descarga la miniatura y la guarda junto al video (mismo nombre, diferente extensiÃ³n).
        """
        try:
            thumbnail_url = job.analysis_data.get('thumbnail')
            if not thumbnail_url:
                print(f"ADVERTENCIA: No se encontrÃ³ miniatura para {job.job_id}")
                return
            ...
            try:
                ...
                job.thumbnail_path = thumbnail_path_smart # OK NUEVO: Guardar en el job
    ...
    def _rebuild_format_maps(self, info: dict) -> tuple[dict, dict]:
        ...
        job_video_formats = {}
        job_audio_formats = {}
        combined_variants = {}  # ðŸ†• Para agrupar variantes multiidioma

        # ðŸ†• PASADA PREVIA: Agrupar variantes combinadas
        for f in formats:
        ...
        # ðŸ†• Filtrar grupos REALMENTE multiidioma (2+ idiomas DIFERENTES)
        ...
        # ðŸ†• Tracking de deduplicaciÃ³n
        combined_keys_seen = set()

        for f in formats:
            format_type = self._classify_format(f)
            
            size_mb_str = "TamaÃ±o desc."
            ...
            if format_type in ['VIDEO', 'VIDEO_ONLY']:  # ðŸ†• Incluir VIDEO_ONLY
                is_combined = acodec != 'none' and acodec is not None
                fps = f.get('fps')
                ...
                if is_combined:
                    ...
                    # ðŸ†• Solo deduplicar si es REALMENTE multiidioma
                ...
                # ðŸ†• [Sin Audio] solo si NO hay audio
                no_audio_tag = ""
                if format_type == 'VIDEO_ONLY':
                    no_audio_tag = " [Sin Audio]"
                
                # ðŸ†• [Multiidioma] solo si es REALMENTE multiidioma
                audio_lang_tag = ""
                ...
                if not compatibility_issues: 
                    ...
                    tags.append("ADVERTENCIA")
                if tags: 
                    label += f" {' '.join(tags)}"
                
                # ðŸ†• Guardar tambiÃ©n quality_key y is_combined
                ...
                if lang_code:
                    ...
                    lang_name = self.host_adapter.LANG_CODE_MAP.get(
                        norm_code, 
                        self.host_adapter.LANG_CODE_MAP.get(norm_code.split('-')[0], lang_code)
                    )
        ...
        # OK NUEVO: Generar opciones de "Extraer Audio" (Copia los videos a la lista de audios)
        for label, video_data in job_video_formats.items():
            if video_data.get('is_combined', False):
                ...
                # IMPORTANTE: Esto evita que job_audio_formats quede vacÃ­o en Twitch Clips
                if audio_extract_label not in job_audio_formats:
                    job_audio_formats[audio_extract_label] = video_data

        # 1. Convertir los dicts a listas de entradas (como en single_context)
        video_entries = []
        for label, data in job_video_formats.items():
            video_entries.append({
                'label': label,
                'format': data, # El 'format' aquÃ­ es el dict de formato completo
        ...
        # 2. Copiar la LÃ“GICA DE ORDENAMIENTO EXACTA de motor individual
        
        # Ordenar Video
        video_entries.sort(key=lambda e: (
            -(e['format'].get('height') or 0),      
            1 if "[Combinado]" in e['label'] else 0, 
            0 if "âœ¨" in e['label'] else 1,         
        ...
        def custom_audio_sort_key(entry):
            ...
            lang_priority = self.host_adapter.LANGUAGE_ORDER.get(
                norm_code, 
                self.host_adapter.LANGUAGE_ORDER.get(norm_code.split('-')[0], self.host_adapter.DEFAULT_PRIORITY)
            )
    ...
    def _resolve_batch_conflict(self, desired_filepath, policy):
        """
        Maneja conflictos de archivo basado en una polÃ­tica.
        """
    ...
    def _predict_final_extension(self, video_info, audio_info, mode):
        """
        Predice la extensiÃ³n de archivo mÃ¡s probable.
        """
        if not video_info: 
            video_info = {}
        if not audio_info: 
    ...
    def _classify_format(self, f):
        ...
        # ðŸ“‹ REGLA 1: GIF explÃ­cito
        if ext == 'gif' or vcodec == 'gif':
            return 'VIDEO'
        
        # ðŸ“‹ REGLA 2: Tiene dimensiones -> VIDEO (con o sin audio)
        if f.get('height') or f.get('width'):
            # ðŸ†• CRÃTICO: Si ambos codecs son 'unknown' o faltan -> ASUMIR COMBINADO
        ...
        # ðŸ†• REGLA 2.5: Livestreams
        if f.get('is_live') or 'live' in format_id:
            return 'VIDEO'
        
        # ðŸ“‹ REGLA 3: ResoluciÃ³n en format_note
        ...
        # ðŸ“‹ REGLA 4: "audio" explÃ­cito en IDs
        if 'audio' in format_id or 'audio' in format_note:
            return 'AUDIO'
        
        # ðŸ†• REGLA 4.5: "video" explÃ­cito en IDs
        ...
        # ðŸ“‹ REGLA 5: ExtensiÃ³n tiene MÃXIMA PRIORIDAD
        if ext in self.host_adapter.AUDIO_EXTENSIONS:
            return 'AUDIO'
        
        # ðŸ†• REGLA 6: Audio sin video (codec EXPLÃCITAMENTE 'none')
        if vcodec == 'none' and acodec and acodec not in ['none', '', 'N/A', 'unknown']:
            return 'AUDIO'
        
        # ðŸ†• REGLA 7: Video sin audio (codec EXPLÃCITAMENTE 'none')
        if acodec == 'none' and vcodec and vcodec not in ['none', '', 'N/A', 'unknown']:
            return 'VIDEO_ONLY'
        
        # ðŸ“‹ REGLA 8: ExtensiÃ³n de video + codecs vÃ¡lidos o desconocidos
        if ext in self.host_adapter.VIDEO_EXTENSIONS:
            if vcodec in ['unknown', ''] and acodec in ['unknown', '']:
                return 'VIDEO'
            return 'VIDEO'
        
        # ðŸ“‹ REGLA 9: Ambos codecs explÃ­citamente vÃ¡lidos
        ...
        # ðŸ“‹ REGLA 10: Protocolo m3u8/dash
        if 'm3u8' in protocol or 'dash' in protocol:
            return 'VIDEO'
        
        # ðŸ†• REGLA 11: Casos de formatos sin codecs claros pero con metadata
        if f.get('tbr') and not f.get('abr'):
            return 'VIDEO'
        elif f.get('abr') and not f.get('vbr'):
            return 'AUDIO'
        
        # ðŸ†• REGLA 12: Fallback para casos ambiguos con extensiÃ³n de video
        if ext in self.host_adapter.VIDEO_EXTENSIONS:
            print(f"ADVERTENCIA ADVERTENCIA: Formato {f.get('format_id')} ambiguo -> asumiendo VIDEO combinado por extensiÃ³n")
            return 'VIDEO'
        
        # ðŸ“‹ REGLA 13: Si llegamos aquÃ­ -> UNKNOWN
        print(f"ADVERTENCIA ADVERTENCIA: Formato sin clasificaciÃ³n clara: {f.get('format_id')} (vcodec={vcodec}, acodec={acodec}, ext={ext})")
        return 'UNKNOWN'
    ...
    def _normalize_info_dict(self, info):
        ...
        # OK INYECCIÃ“N DEL PARCHE (Igual que en single_context y batch_context)
        # Asegura que el procesador vea cÃ³decs vÃ¡lidos y no falle al clasificar.
        ...
        elif ext in self.host_adapter.AUDIO_EXTENSIONS:
            is_audio_content = True
        ...
        if is_audio_content:
            print(f"DEBUG: ðŸŽµ Contenido de audio directo detectado (ext={ext})")
            ...
            print(f"DEBUG: OK Formato sintÃ©tico creado")
    ...
    def _find_preset_params(self, preset_name):
        ...
        Adaptado de batch_download_tab.py, busca en single_context.
        """
        # Buscar en personalizados
        for preset in getattr(self.host_adapter.single_context, 'custom_presets', []):
            if preset.get("name") == preset_name:
                return preset.get("data", {})
        
        # Buscar en integrados
        if preset_name in self.host_adapter.single_context.built_in_presets:  
            return self.host_adapter.single_context.built_in_presets[preset_name]
            
        return {}

    def _get_job_media_duration(self, job: Job, input_file: str) -> float:
        """
        Obtiene la duraciÃ³n del medio, priorizando los datos del anÃ¡lisis
        y usando ffprobe como fallback.
        """
        # 1. Prioridad: Datos del anÃ¡lisis (mÃ¡s rÃ¡pido)
        if job.analysis_data:
            duration = job.analysis_data.get('duration')
            if duration:
                return float(duration)
        
        # 2. Fallback: Usar ffprobe (mÃ¡s lento pero preciso)
        try:
            media_info = self.host_adapter.ffmpeg_processor.get_local_media_info(input_file)
            if media_info:
                return float(media_info['format']['duration'])
        except Exception as e:
            print(f"ADVERTENCIA: No se pudo obtener duraciÃ³n con ffprobe: {e}")
    ...
    def _execute_recode_master(self, job: Job, input_file, output_dir, base_filename, recode_options):
        """
        FunciÃ³n maestra que maneja la lÃ³gica de recodificaciÃ³n para un job.
        Adaptada de motor individual.
        """
        final_recoded_path = None
        backup_file_path = None
        
        try:
            self.event_callback(job.job_id, "RUNNING", "Preparando recodificaciÃ³n...")
            ...
            # --- INICIO DE CORRECCIÃ“N (Muxer vs Contenedor) ---
            container_ext = recode_options.get('recode_container', '.mp4')
            if container_ext == "-":
                container_ext = ".mp4"
            
            # Buscar un muxer especÃ­fico en el mapa (ej: .m4a -> mp4)
            # Usamos self.host_adapter.FORMAT_MUXER_MAP
            muxer_name = self.host_adapter.FORMAT_MUXER_MAP.get(container_ext, container_ext.lstrip('.'))
            ...
            if recode_options['mode_compatibility'] != "Solo Audio":
                if recode_options["recode_video_enabled"]:
                    ...
                    codec_db = self.host_adapter.ffmpeg_processor.available_encoders[proc]["Video"]
                    ...
                    if profile_params_list == "CUSTOM_GIF":
                        try:
                            ...
                            raise Exception("Valores de FPS/Ancho para GIF no son vÃ¡lidos.")
                    ...
                    # Filtros de video (FPS y resoluciÃ³n)
                    video_filters = []
                    ...
                    if recode_options.get("resolution_change_enabled"):
                        # (La lÃ³gica de preset/personalizado ya estÃ¡ resuelta en el preset)
                        try:
                            ...
                            pass # Ignorar si los valores de res estÃ¡n vacÃ­os
            ...
            if not is_gif_format:
                ...
                if is_pro_video_format:
                    ...
                    audio_codec_db = self.host_adapter.ffmpeg_processor.available_encoders["CPU"]["Audio"]
            ...
            # --- INICIO DE CORRECCIÃ“N (Soporte Multipista Local v2) ---
            
            # 1. Determinar el Ã­ndice de audio
            ...
            if not is_gif_format:
                ...
                if job.job_type == "LOCAL_RECODE":
                    ...
                    if use_all_tracks:
                        selected_audio_idx = "all"
                        print(f"DEBUG: [RecodificaciÃ³n Local] Usando 'all' pistas de audio (checkbox activado).")
                    else:
                        # Checkbox no activado, buscar el Ã­ndice especÃ­fico guardado
                        saved_idx = job.config.get('resolved_audio_stream_index')
                        if saved_idx is not None:
                            selected_audio_idx = int(saved_idx)
                            print(f"DEBUG: [RecodificaciÃ³n Local] Usando pista de audio especÃ­fica: {selected_audio_idx}")
                        else:
                            # Fallback: no hay Ã­ndice guardado, usar la primera pista de audio (Ã­ndice 0)
                            # Esto es mÃ¡s seguro que "all" para evitar el error del muxer
                            try:
                                audio_streams = job.analysis_data.get('local_info', {}).get('audio_streams', [])
                                if audio_streams:
                                    selected_audio_idx = audio_streams[0].get('index', 0)
                                    print(f"ADVERTENCIA: [RecodificaciÃ³n Local] No se encontrÃ³ Ã­ndice de audio guardado. Usando fallback a la primera pista (Ã­ndice {selected_audio_idx}).")
                                else:
                                    selected_audio_idx = None # No hay audio
                            except Exception:
                                selected_audio_idx = 0 # Fallback final
                
            # 2. Determinar el Ã­ndice de video (asumir el primero si existe)
            ...
            # FunciÃ³n de callback de progreso para este job
            def recode_progress_callback(percentage, message):
                self.event_callback(job.job_id, "RUNNING", message)

            # Ejecutar recodificaciÃ³n
            self.host_adapter.ffmpeg_processor.execute_recode(
                command_options, 
    ...
    def _download_best_thumb_png(self, entry, output_dir, title):
        ...
        try:
            ...
            # Primero intentamos descargar la versiÃ³n HD forzada.
            # Si falla (404), caemos a la versiÃ³n original (hqdefault).
            final_url = thumb_url
            if "i.ytimg.com" in thumb_url:
                import re
                max_res_url = re.sub(r'/(hq|mq|sd|default)default', '/maxresdefault', thumb_url)
                
                # Probamos si maxres existe (haciendo una peticiÃ³n HEAD o GET rÃ¡pida)
                try:
                    ...
                    pass # Si falla la comprobaciÃ³n, usar la original
            ...
            sanitized_title = self.host_adapter.single_context.sanitize_filename(title)
            ...
            return output_path # <--- AÃ‘ADIDO: Devolver la ruta
            
        except Exception as e:
            print(f"ADVERTENCIA: FallÃ³ descarga de miniatura PNG para '{title}': {e}")
            return None # <--- AÃ‘ADIDO: Devolver None si falla
